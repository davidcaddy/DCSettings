...
- The default settings UI renders controls for `Bool`, `Int`, `Double`, `String`, `Date`, and `Color`. It renders `options` for `Int`, `Double`, and `String`; `bounds` for `Int`, `Double`, and `Date`; and `step` for `Int` controls and bounded `Double` sliders. Other configuration combinations and custom value types still validate values, but they need custom UI if users should edit them directly.
